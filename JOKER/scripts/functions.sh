...
# Cleans Module Directory 
cleanup() {
  rm -rf $MODPATH/JOKER 2>/dev/null
}
...
# Extract files
unzip -o "$ZIPFILE" -x 'META-INF/*' 'JOKER/functions.sh' -d $MODPATH >&2
[ -f "$MODPATH/JOKER/addon.tar.xz" ] && tar -xf $MODPATH/JOKER/addon.tar.xz -C $MODPATH/JOKER 2>/dev/null

# Run addons
if [ "$(ls -A $MODPATH/JOKER/addon/*/install.sh 2>/dev/null)" ]; then
  ui_print " "; ui_print "- Running Addons -"
  for i in $MODPATH/JOKER/addon/*/install.sh; do
    ui_print "  Running $(echo $i | sed -r "s|$MODPATH/JOKER/addon/(.*)/install.sh|\1|")..."
    . $i
  done
fi
...
### Install
[ -f "$MODPATH/joker.sh" ] && . $MODPATH/joker.sh
